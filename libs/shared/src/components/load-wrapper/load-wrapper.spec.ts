...
import { LoadWrapper, ContentContext, ErrorContext, ReloadingContext, IdleContext } from './load-wrapper';
...
function createMockHttpData<T>(initial?: {
	...
	return {
		status: signal<ResourceStatus>(initial?.status ?? 'idle'),
...
@Component({
	...
	imports: [LoadWrapper, JsonPipe],
	template: `
		<load-wrapper [source]="$any(source())">
...
@Component({
	...
	imports: [LoadWrapper, CommonModule, JsonPipe],
	template: `
		<load-wrapper
			[source]="$any(source())"
			[emptyWhen]="emptyWhen()"
...
function queryAllEl(fixture: ComponentFixture<unknown>, selector: string): HTMLElement[] {
...
describe('LoadWrapper', () => {
	...
	describe('with default templates', () => {
		...
		let mock: MockHttpData<string[]>;

		beforeEach(async () => {
			mock = createMockHttpData<string[]>();
			...
			host.source.set(mock as unknown as HttpData<string[]>);
			await stabilize(fixture);
		});

		it('should create the component', () => {
			const wrapper = queryEl(fixture, 'load-wrapper');
		...
		it('should show default reloading overlay when reloading with showReloadingState=true', async () => {
			// First resolve with data so contentContext is available
			mock.status.set('resolved');
			mock.value.set(['data']);
			mock.hasValue.set(true);
			await stabilize(fixture);

			// Now switch to reloading
	...
	describe('emptyWhen', () => {
		let fixture: ComponentFixture<TestHostAllTemplatesComponent>;
		let host: TestHostAllTemplatesComponent;
		let mock: MockHttpData<{ items: string[] }>;

		beforeEach(async () => {
			mock = createMockHttpData<{ items: string[] }>();
			...
			fixture = TestBed.createComponent(TestHostAllTemplatesComponent);
			host = fixture.componentInstance;
			host.source.set(mock as unknown as HttpData<{ items: string[] }>);
			await stabilize(fixture);
		});
	...
	describe('without content template', () => {
		...
		it('should render nothing for resolved state without content template', async () => {
			...
			// No content template means nothing renders for resolved state
			expect(queryEl(fixture, '.data-status__loading')).toBeNull();
			expect(queryEl(fixture, '.data-status__error')).toBeNull();
		});
	});
